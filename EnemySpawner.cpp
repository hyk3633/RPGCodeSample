...
// 공격 대상으로 향하는 플로우 벡터를 반환
FVector* AEnemySpawner::GetFlowVector(ACharacter* TargetCharacter, ACharacter* EnemyCharacter)
{
	const int32 Idx = GetConvertCurrentLocationToIndex(EnemyCharacter->GetActorLocation()); // 에너미 위치를 인덱스로 변환
	FFlowVector* FlowVector = TargetsFlowVectors.Find(TargetCharacter); // TMap에서 공격 대상의 플로우 벡터 배열을 가져옴
	if (FlowVector) return &FlowVector->GridFlows[Idx]; // 에너미의 현재 위치에 해당하는 Flow 벡터를 반환
	else return nullptr;
}
	...
	*(DistScore + DestIdx) = 0;
	...
	while (!Next.IsEmpty())
	{
		...
		for (int8 Idx = 0; Idx < 8; Idx++)
		{
			...
			{
				if (*(DistScore + NextIdx) != -1) continue; // 이미 계산한 그리드일 경우 
				if (IsMovableArr[NextIdx] == false) continue; // 통행 불가 그리드일 경우
